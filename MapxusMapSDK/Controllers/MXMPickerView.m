...
#import "MXMPickerContainerView.h"
#import "MXMPickerTextRenderView.h"

static CGFloat kMXMPickerViewDefaultRowHeight = 44;
static CGFloat kMXMPickerViewDefaultSpacingOfComponents = 10;

@interface MXMPickerView() <MXMPickerContainerViewDelegate,MXMPickerContainerViewDateSource>

@property (strong, nonatomic) UIView *selectionIndicatorView;

@property (nonatomic, strong) NSMutableArray<MXMPickerContainerView *> *containers;
...
- (NSMutableArray<MXMPickerContainerView *> *)containers
{
    if (!_containers) {
...
- (void)setDataSource:(id<MXMPickerViewDataSource>)dataSource
{
...
- (void)setSelectionIndicatorStyle:(MXMPickerViewSelectionIndicatorStyle)selectionIndicatorStyle
{
    ...
    if (self.selectionIndicatorView) {
        ...
        if (selectionIndicatorStyle == MXMPickerViewSelectionIndicatorStyleDefault) {
            
            [self setupSelectionIndicatorStyleDefault];
            
        } else if (selectionIndicatorStyle == MXMPickerViewSelectionIndicatorStyleDivision) {
            
            [self setupSelectionIndicatorStyleDivision];
            
        } else if (selectionIndicatorStyle == MXMPickerViewSelectionIndicatorStyleCustom) {
...
{
    _selectionIndicatorStyle = MXMPickerViewSelectionIndicatorStyleDefault;
    ...
    _spacingOfComponents = kMXMPickerViewDefaultSpacingOfComponents;
}
...
- (void)setupContainerView
{
    ...
    for (NSUInteger i = 0; i < components; i++) {
        ...
        MXMPickerContainerView *container = [[MXMPickerContainerView alloc] init];
...
- (void)setupVerticalDivisionLine
{
    ...
    for (NSUInteger i = 0; i < components - 1; i++) {
        MXMPickerContainerView *containerView = self.containers[i];
...
- (void)setupSelectionIndicatorStyleDivision
{
    ...
    for (NSUInteger i = 0; i < 2; i++) {
        ...
        for (MXMPickerContainerView *containerView in self.containers) {
...
- (UIView *)containerView:(MXMPickerContainerView *)containerView viewForRow:(NSInteger)row reusingView:(UIView *)view
{
    ...
    if (!self.isItemTypeText) {
        ...
        MXMPickerTextRenderView *textView = (MXMPickerTextRenderView *)view;
        
        if (!textView) {
            textView = [[MXMPickerTextRenderView alloc] initWithFrame:CGRectMake(0, 0, containerView.frame.size.width, [self rowHeightForComponent:component])];
...
- (void)containerViewDidScroll:(MXMPickerContainerView *)containerView
{
    ...
    if (self.isItemTypeText) {
        ...
        [containerView.visibleItemViews enumerateObjectsUsingBlock:^(MXMPickerTextRenderView  *_Nonnull textView, NSUInteger idx, BOOL * _Nonnull stop) {
...
#pragma mark - tools
- (void)renderTextViewInRow:(NSInteger)row forContainerView:(MXMPickerContainerView *)containerView
{
    MXMPickerTextRenderView *textView = (MXMPickerTextRenderView *)[containerView viewForRow:row];
...
- (CGFloat)selectionIndicatorHeight
{
    ...
    if ([self.delegate respondsToSelector:@selector(pickerView:rowHeightForComponent:)]) {
        ...
        indicatorHeight = kMXMPickerViewDefaultRowHeight;
    }
...
- (CGFloat)rowHeightForComponent:(NSUInteger)component
{
    ...
    return kMXMPickerViewDefaultRowHeight;
}
...
- (void)reloadAllComponents {
    ...
    for (MXMPickerContainerView *container in self.containers) {
