...
@implementation UploadLogManager
...
+ (instancetype)sharedInstance{
    ...
    static UploadLogManager *instance = nil;
    
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        if (!instance) {
            instance = [[UploadLogManager alloc] init];
        }
    });
...
- (instancetype)init{
    ...
    if (self) {
        ...
        // 日志的目录路径
        self.basePath = [NSString stringWithFormat:@"%@%@/",NSHomeDirectory(), @"/Documents/OTKLog/"];
    }
    return self;
}
...
#pragma mark - 写入日志
    ...
    // 异步执行
    dispatch_queue_t queue = dispatch_queue_create("writelog", DISPATCH_QUEUE_SERIAL);
    ...
    dispatch_sync(queue, ^{
        ...
        // [时间]-[模块]-日志内容
        NSString* timeStr = [self.timeFormatter stringFromDate:[UploadLogManager getCurrDate]];
...
- (void)clearExpiredLog {
    ...
    for (NSString *file in files) {
        ...
        if (date) {
            ...
            NSTimeInterval currTime = [[UploadLogManager getCurrDate] timeIntervalSince1970];
